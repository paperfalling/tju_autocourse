...
SSO 沿用历史 TJU CAS 协议：向 `learning.twt.edu.cn/enc` 发送学号、密码与登录令牌的拼接值，向同站点 `/ocr` 发送验证码图片。这是外部辅助服务依赖；CAS/EAMS 与辅助服务使用独立会话。验证码识别、协议变化或辅助服务故障可能导致登录失败。

### 从旧配置迁移

旧的用户顶层 `cookie`、`username`、`password` 字段不再接受。将 Cookie 移入 `auth: {type: cookie, cookie: ...}`，或将账密移入 `auth: {type: sso, username: ..., password: ..., retries: 2}`。不要同时保留旧字段。其他现有业务字段维持名称与含义。
...
旧 Python 异步接口、`set_config_meta`、全局配置与 `user_models` 已移除。程序调用使用 `load_config(path)` 返回的类型化对象及同步 `User`，主入口仍为 `run(config_path)`。`run` 返回是否所有用户正常完成；命令在配置或用户运行失败时返回非零退出码。
...
`skipPre=true` 仅绕过余量检查。开放后余量可能不能实时更新，需要绕过旧快照时由用户显式开启；程序不按钟点自动切换。例如设置 12 点开始并开启 `skipPre`，可以避免旧快照在请求前淘汰目标。要对快照已满的课程使用 `full: retry`，也需要开启 `skipPre`。
